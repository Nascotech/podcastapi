...
let PublisherCronjob = {
        ...
        syncPodcastList: async function (request, response) {
            ...
            async function fetchPodcast(userInfo, collectionInfo, pageNo) {
                try {
                    ...
                    if (res.status === 200 && contentType && contentType.indexOf("application/json") !== -1) {
                        ...
                        console.error(" ⚠️ ********* Response error during podcast fetch list", res)
                        if (res.status === 401) {
                            await updateAccessToken(userInfo);
                            return {data: []};
                        } else {
                    ...
                    console.error(" ⚠️ ********* Error while fetchPodcast list", e)
                }


            }

            async function fetchEpisode(userInfo, podcast, pageNo) {
                ...
                try {
                    ...
                    if (res.status === 200 && contentType && contentType.indexOf("application/json") !== -1) {
                        ...
                        console.error(" ⚠️ ********* Response error fetchEpisode list", res)
                        if (res.status === 401) {
                            await updateAccessToken(userInfo);
                            return {data: []};
                        } else {
                    ...
                    console.error(" ⚠️ ********* Error while fetchEpisode list", e)
                }
            ...
            function removeEpisodeFromDatabase(episodeList, podcast) {
                return new Promise(function (resolve, reject) {
                    let count = 0;
                    episodeList.forEach(async episode => {
                        await EpisodesModel.deleteOne({
                            publisher: podcast.publisher,
                            sgPodcastId: podcast.podcastId,
                            'guid.value': episode.guid.value
                        });
                        count++;
                        if (episodeList.length == count) {
                            return resolve(true);
            ...
            try {
                ...
                let role = await RolesModel.findOne({slug: varConst.PUBLISHER})
                ...
                async function sync() {
                    ...
                    try {
                        ...
                        user = await UserModel.findOne({
                            ...
                            $or: [{lastSyncDate: {$lt: syncDate}}, {lastSyncDate: {$exists: false}}]
                        })
                        if (user) {
                            user.lastSyncDate = syncDate
                            console.log(` 👤 ********************* SYNC Start for User ${user.sgUsername} *********************`)
                            let collectionList = await fetchCollection(user);
                            let oldPodcastArr = await fetchOldPodcastList(user);
                            for (const collection of collectionList) {
                                await fetchPodcastList(user, collection, oldPodcastArr);
                            }
                            user.lastSyncDate = syncDate
                            await user.save();
                            console.log(` ✅  ********************* SYNC End for User ${user.sgUsername} *********************`)
                            await sync()
                        } else {
    ...
    return new Promise(function (resolve, reject) {
        ...
        fetch(url, {method: 'POST', body: form}).then((res) => {
            ...
            if (res.status == 200 && contentType && contentType.indexOf("application/json") !== -1) {
                ...
                console.error(" ❌ ********************* Error while updating refresh token")
                // console.error(userInfo.publisherName)
            ...
            if (result) await UserModel.updateOne({'_id': userInfo.id}, {
                $set: {
                    "sgTokenType": result.token_type,
                    "sgAccessToken": result.access_token,
                    "sgRefreshToken": result.refresh_token
                }
            });
            resolve(result);
        }).catch(err => {
...
function updatePublisherGroups() {
    return new Promise(function (resolve, reject) {
        ...
        function updateUserGroups(groupList, userInfo) {
            return new Promise(async function (resolve, reject) {
                let groupFiltered = await groupList.filter((el) => {
                    return userInfo.podcastsGroups.some((f) => {
                        return f == el.id;
                    });
                });
...
function updatePodcastImages() {
    return new Promise(function (resolve, reject) {
        ...
        function getAllPodcastImages() {
            return new Promise(function (resolve, reject) {
                PodcastsModel.find({
                    "imageSync": varConst.INACTIVE,
                    'image': {$exists: true, $ne: null}
                }, {publisher: true, podcastId: true, image: true}).then(result => {
                    resolve(result);
                }).catch(err => {
                    if (err) reject(err);
                });
            ...
            return new Promise(function (resolve, reject) {
                if (list.length > 0) {
                    ...
                    list.forEach(async info => {
                      let mainDir = masterConfig['isStaging'] ? "staging/uploads/publisher_" : "uploads/publisher_";
                        ...
                        sharp(response.data).resize(240, 240).toBuffer().then(async result => {
                            ...
                            await s3.upload(params, async function (s3Err, data) {
                                if (s3Err) {
                                    ...
                                    await PodcastsModel.updateOne({"_id": modelId}, {
                                        $set: {
                                            "image": data.Location,
                                            'imageSync': varConst.ACTIVE
                                        }
                                    });
                                    resolve(true);
                                }
